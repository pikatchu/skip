...
# (cd ./third-party/jemalloc/src && ./configure CC=$CLANG CXX=$CLANGXX --prefix=$TPDIR --with-mangling)
...
# (cd third-party/pcre/src/ && autoreconf -f -i && CC=$CLANG CXX=$CLANGXX ./configure --prefix="$TPDIR" --enable-static --disable-shared --enable-utf && make -j 16 && make install)
